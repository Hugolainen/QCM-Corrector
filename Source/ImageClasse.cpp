	...
	if (this->lireNbRegions() > 0) {
		...
		for (int num = 1; num <= this->lireNbRegions();num++) {
			...
			while (!retour) {
				...
				/*
								int size_freemanHistoLearn = tab[k].codeFreeman.length();
								for each (char c  in VecSig.at(1).codeFreeman) {
									freemanHistoLearn[c - '0']++;
								}
								for (int i = 0; i < 8; i++)
		...
		// Calcul histos freeman
		for (int num = 1; num <= this->lireNbRegions(); num++) {
			int size_freemanHisto = tab[num].codeFreeman.length();
			for each (char c  in tab[num].codeFreeman) {
				tab[num].freemanHist[c - '0']++;
			}
			for (int i = 0; i < 8; i++)
				tab[num].freemanHist[i] = tab[num].freemanHist[i] / (float)size_freemanHisto;
		}
...
// cellules de Voronoï ou moyen "efficace" de boucher les trous d'une image issue de filtrage
...
/*
CImageNdg CImageClasse::masquage(int valR, int valG, int valB)
{
	CImageNdg out(this->lireHauteur(), this->lireLargeur, 0);
	out.ecrireNom(this->lireNom() + "Ma");

	for (int i = 0; i < this->lireNbPixels(); i++)
		if (this->operator(i)[0] == valR)
		{
			out(i) = 1;
		}
	return out;
...
*/
// Arg = img init 
CImageCouleur CImageClasse::covar(const CImageCouleur& imgInit) {
	CImageCouleur out(imgInit);
	/*
	CImageCouleur mask(imgInit);
	for (int p = 0; p < img.lireNbPixels(); p++)
		if (pts(p) >= 1)
		{
			mask(p)[0] = 255;
			mask(p)[1] = 0;
	...
	*/
	...
	for (int i = 0; i < this->lireHauteur() - 1; i++) {
		for (int j = 0; j < this->lireLargeur() - 1; j++) {
			if (this->operator()(i, j) == 255) {							// Si object detecté
				sum_i += (double)i;
				sum_j += (double)j;
				sum_ii += (double)i*(double)i;
				sum_jj += (double)j*(double)j;
				sum_ij += (double)i*(double)j;
				surface++;
			}
		}
	}
	//coeff_a = (sum_ij - (sum_i / sum_ii) - (sum_j / sum_jj)) /( (double)surface);
	coeff_a = (sum_jj - sum_i*sum_j) / ((sum_ii-(sum_i*sum_i )/(surface*surface)*(sum_ii - (sum_i*sum_i) / (surface*surface))));
	coeff_b = sum_j / surface - coeff_a*(sum_i / surface);
	//printf("i=%.1lf, j=%.1lf, ii=%.1lf, jj=%.1lf, ij=%.1lf\n", sum_i, sum_j, sum_ii, sum_jj,sum_ij);
	printf("a = %.1lf, b = %.1lf\n", coeff_a, coeff_b);

	for (int i = 0; i < this->lireHauteur() - 1; i++) {
	//	out(i, (int)coeff_a*i + (int)coeff_b)[0] = 255;
	//out(i, (int)coeff_a*i + (int)coeff_b)[1] = 0;
	//	out(i, (int)coeff_a*i + (int)coeff_b)[2] = 0;
		//out(i, j)[0] = 0;
		//out(i, j)[1] = 0;
		//out(i, j)[2] = 0;
